...
const EditBoard: React.FC<EditBoardProps> = ({ isEditBoardOpen, toggleEditBoard }) => {
    ...
    return (
        <dialog open={isEditBoardOpen} onClick={() => toggleEditBoard()} className="bg-[#000] bg-opacity-60 min-h-screen w-screen absolute top-0 py-48 z-10">
            <article className="bg-white w-[21.4375rem] p-6 rounded-lg mx-auto dark:bg-darkGray md:w-[30rem]" onClick={stopElementEventPropagation}>
                <h2 className="text-darkBlack text-lg font-bold mb-6">Edit board</h2>
                <form>
                    ...
                    <button className='bg-purple text-white h-10 text-semibold text-[0.8125rem] w-full rounded-[1.25rem]'>Save Changes</button>
                </form>
            </article>
        </dialog>
    )
